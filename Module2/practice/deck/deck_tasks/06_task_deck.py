...
Создайте две колоды по 52 карты. Перемешайте их вместе - в итоге получится
одна колода из 104 карт. Выбросите/вытяните половину карт. Узнайте,
какой/каких мастей в колоде осталось больше всего?"""
...
deck.draw(len(deck) // 2)
count = dict.fromkeys(Card.SUITS, 0)
for card in deck:
    count[card.suit] += 1

max_cards = max(count.values())
print('В оставшейся половине двойной колоды масти(ей) ', end='')
