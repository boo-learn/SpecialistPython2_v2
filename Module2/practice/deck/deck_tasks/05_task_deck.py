...
print(f"p_2 cards\n{p2_hand}")
print()
p1_hand = p1_hand.sort_by_value()
cards_on_table = Deck(seq=p1_hand.draw(1))
print(f"p_1 -> table: {cards_on_table[-1]}")

player_is_lost = True
for card in p2_hand:
    if card.suit == cards_on_table[-1].suit and card.value > cards_on_table[-1].value:
        cards_on_table.append(card)
        p2_hand.remove(card)
        print(f"p_2 -> table: {cards_on_table[-1]}")
        player_is_lost = False
        break
print()
print(f"p_1 cards\n{p1_hand}")
print(f"p_2 cards\n{p2_hand}")
...
if player_is_lost:
    ...
    player_is_lost = True
    for t_card in cards_on_table:
        for card in p1_hand:
            if t_card.value == card.value:
                cards_on_table.append(card)
                p1_hand.remove(card)
                print(f"p_1 -> table: {cards_on_table[-1]}")
                player_is_lost = False
