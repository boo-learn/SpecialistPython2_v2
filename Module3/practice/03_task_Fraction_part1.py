class Fraction:
    ...
    def __add__(self, fraction):
        ...
        den_sum = self.denominator + fraction.denominator
        whole = abs(num_sum) // den_sum
        new_numerator = abs(num_sum) % den_sum
        if num_sum < 0 and whole:
            whole = -whole
        elif num_sum < 0:
            new_numerator = -new_numerator
        return f"{whole if whole else ''} {new_numerator}/{den_sum}"
