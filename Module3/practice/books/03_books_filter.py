...
class Book:
    ...
    def to_str(self) -> str:
        return f"Наименование: «{self.name}». Автор: {self.author.short_name()}. Год публикации:{self.year}"
...
for book in books_catalog:
    if book.author.surname == surname.title():
        find_author += 1
        print(f"{find_author}. {book.to_str()}")
if find_author == 0:
