...
def quick_sort(list_to_sort: list):
    ...
    def quick_sort_main(nums):
        # Создадим вспомогательную функцию, которая вызывается рекурсивно
        def _quick_sort_main(items, low, high):
            if low < high:
                # Индекс опорного элемента
                split_index = partition(items, low, high)
                _quick_sort_main(items, low, split_index)
                _quick_sort_main(items, split_index + 1, high)

        _quick_sort_main(nums, 0, len(nums) - 1)

    quick_sort_main(list_to_sort)
