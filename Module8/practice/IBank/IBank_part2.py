...
class Operation:
    ...
    def __init__(self, amount: int, oper_type: str, client: str = ''):
        self.type = oper_type
        self.amount = amount
        self.client = client
    ...
    def __repr__(self) -> None:
        ...
        """
        return f'{self.type} {self.amount} руб. {self.client}'.strip()
...
class Account:
    ...
    def transfer(self, target_account: 'Account', amount: int, to_history: bool = True) -> None:
        ...
        if to_history:
            self.__history.append(Operation(amount, Operation.TRANSFER_OUT, f'на счет клиента {target_account.name}'))
            self.__history.append(Operation(amount, Operation.TRANSFER_IN, f'со счета клиента {self.name}'))
