...
class Operation:
    ...
    def __init__(self, amount: int, oper_type: str, target_account = None, commission = None):
        self.type = oper_type
        self.amount = amount
    ...
    def __repr__(self) -> None:
        ...
        if self.type == Operation.WITHDRAW:
            return f'{self.type} {self.amount} руб. (комиссия {self.commission * self.amount} руб.)'
        elif self.type == Operation.TRANSFER_IN:
            return f'{self.type} {self.amount} руб. со счета клиента: {self.target_account.name}'
        else:
            return f'{self.type} {self.amount} руб. на счет клиента: {self.target_account.name} (комиссия {self.commission * self.amount} руб.)'
...
class Account:
    ...
    def withdraw(self, amount: int, to_history: bool = True) -> None:
        ...
        withdraw_amount = int(amount * (1 + self.commission))

        if self.__balance < withdraw_amount:
            raise ValueError("Недостаточно средств на счете. Операция отклонена")
        self.__balance -= withdraw_amount
        if to_history:
            self.__history.append(Operation(amount, Operation.WITHDRAW, self.commission))
    ...
    def transfer(self, target_account: 'Account', amount: int, to_history: bool = True) -> None:
        ...
        if to_history:
            self.__history.append(Operation(amount, Operation.TRANSFER_OUT, target_account, self.commission))
            target_account.__history.append(Operation(amount, Operation.TRANSFER_IN, self, self.commission))
