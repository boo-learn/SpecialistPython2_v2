...
class Operation:
    ...
    def __repr__(self) -> str:
        ...
        if self.type == Operation.TRANSFER_OUT:
            return f"{Operation.TRANSFER_OUT} {self.amount} руб. на счет клиента: {self.target_account.name}"
...
class Account:
    ...
    def withdraw(self, amount: int, to_history: bool = True) -> None:
        ...
        :param amount: сумма
        """
        if self.__balance - amount*Account.COMMISION  < amount:
            raise ValueError("Недостаточно средств на счете. Операция отклонена")

        self.__balance -= amount + int(amount * Account.COMMISION)
        if to_history:
