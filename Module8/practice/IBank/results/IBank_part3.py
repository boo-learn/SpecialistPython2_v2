...
class Operation:
    ...
    def __repr__(self):
        add_str = ''
        if self.operation == "СНЯТИЕ":
            add_str = f' (комиссия: {round(self.amount*0.02)} руб.)'
        if self.operation == "ПЕРЕВОД":
            add_str = f' (комиссия: {round(self.amount*0.02)} руб.) на счет клиента: {self.account.name}'
...
class Account:
    ...
    def withdraw(self, amount, record_to_history = True):
        ...
        :param amount: сумма
        """
        if self.__balance - amount*1.02 < 0:
            raise ValueError('Недостаточно средств на счету')
        self.__balance -= amount*1.02
        if record_to_history:
