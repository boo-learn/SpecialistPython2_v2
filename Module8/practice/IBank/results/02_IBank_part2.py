...
class Account(AccountBase):
    ...
    def __init__(self, name, passport8, phone_number, start_balance=0):
        ...
        if re.fullmatch('\+7\d{10}', phone_number):
            self.phone_number = phone_number
        else:
            raise ValueError('Телефон должен быть введен в формате "+7xxx-xxx-xx-xx"')
        self.balance = start_balance
        self.history = []
        str_history = f'{datetime.datetime.now()} Начальный баланс {start_balance}'
    ...
    def deposit(self, amount):
        self.not_closed()
        self.balance += amount
        str_history = f'{datetime.datetime.now().strftime("%d-%m-%Y %H:%M")} Пополнение на сумму {round(amount)}, баланс {round(self.balance)}'
        self.history.append(str_history)
    ...
    def withdraw(self, amount, limit=0):
        self.not_closed()
        commission_rate = 0.05 if self.balance < 0 else 0.02
        ...
        if amount + commission > self.balance + limit:
            ...
            str_history = f'{datetime.datetime.now().strftime("%d-%m-%Y %H:%M")} Списание суммы {round(amount)}, баланс {round(self.balance)}'
            self.history.append(str_history)
            str_history = f'{datetime.datetime.now().strftime("%d-%m-%Y %H:%M")} Списание комиссии {round(commission)}, баланс {round(self.balance)}'
            self.history.append(str_history)
    ...
    def close(self):
        ...
        if self.balance:
            ...
            self.withdraw(self.balance / (1 + commission_rate))
        self.closed = True
        str_history = f'{datetime.datetime.now().strftime("%d-%m-%Y %H:%M")} Счет закрыт'
        self.history.append(str_history)
...
class CreditAccount(Account):
    ...
    def __init__(self, name, passport8, phone_number, start_balance=0, negative_limit=0):
        ...
        self.balance = start_balance
        self.history = []
        str_history = f'{datetime.datetime.now().strftime("%d-%m-%Y %H:%M")} Начальный баланс {start_balance}'
        self.history.append(str_history)
        self.closed = False
        self.limit = negative_limit
        str_history = f'{datetime.datetime.now().strftime("%d-%m-%Y %H:%M")} Максимальный кредит {negative_limit}'
        self.history.append(str_history)
...
#
account_ivan = CreditAccount(name="Иван", passport8="23451678", phone_number="+79119000000", start_balance=0, negative_limit=1000)
account_vasya = Account(name="Василий", passport8="12345679", phone_number="+79118000000", start_balance=1000)
