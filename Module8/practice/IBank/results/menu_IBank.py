...
def close_account():
    ...
    if type(acc) is iBank.Account:
        iBank.Account.to_archive(acc)
    if type(acc) is iBank.CreditAccount:
        iBank.CreditAccount.to_archive(acc)
...
def create_new_account():
    ...
    start_balance = int(input("Начальная сумма на счету: "))
    new_acc = iBank.Account(name, passport, phone_number, start_balance=start_balance)
    for account in accounts:
...
def create_new_credit_account():
    ...
    new_acc = iBank.CreditAccount(name, passport, phone_number,
                                  start_balance=start_balance, negative_limit=negative_limit)
    for account in accounts:
        if account.passport8 == new_acc.passport8 or account.phone_number == new_acc.phone_number:
            raise ValueError('Аккаунт с данным паспортом либо телефоном уже существует')
    accounts.append(new_acc)
...
def reopen_account():
    ...
    if not is_found:
        raise ValueError('Аккаунта с данным паспортом не существует')
    if type(acc) is iBank.Account:
        iBank.Account.restore(acc)
    if type(acc) is iBank.CreditAccount:
        iBank.CreditAccount.restore(acc)
