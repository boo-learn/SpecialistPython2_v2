...
class Operation:
    ...
    def __str__(self):
        if self.type == Operation.OPEN:
            return f'Открытие счета'
        if self.type == Operation.CLOSE:
            return f'Закрытие счета'
        if self.type == Operation.REOPEN:
            return f'Переоткрытие счета'
        if self.target is not None:
            target_phone = self.target.phone_number
        else:
            target_phone = ''
        if self.fee == 0:
            return f'Операция {self.type} {target_phone} на сумму {self.amount}'
        return f'Операция {self.type} {target_phone} на сумму {self.amount} с комиссией {self.fee * self.amount:.2f}'
...
class Account(AccountBase):
    ...
    def account_close(self):
        ...
        op = Operation(Operation.CLOSE, None)
        self.history.append(op)
        self.is_archived = True

    @staticmethod
    def __validate(passport8, phone_number):
        int(passport8)
