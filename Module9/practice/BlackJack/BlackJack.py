...
from dect_classes_2 import Deck, Card
...
def sum_points(cards):
    ...
    sum_points = 0
    for card in cards:
        sum_points += Deck.weight_card[Deck.values.index(card.value)]
    ...
    if sum_points > 21:
        sum_points = 0
        for card in cards:
            sum_points += Deck.weight_card_A[Deck.values.index(card.value)]
